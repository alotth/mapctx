...
function createTaskInfoElement(task) {
  ...
  const subIssueInfo = task.subIssueProgress
    ? `<div class="task-info-item">
         <span class="task-info-label">Sub-issue:</span>
         <span>${task.subIssueProgress}</span>
       </div>`
    : ''
  ...
  return `<div class="task-info">${typeInfo}${parentInfo}${subIssueInfo}${startInfo}${dueInfo}${workloadInfo}</div>`
}
