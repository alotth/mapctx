...
import { parseTaskDetailMarkdown as parseTaskDetailFromCore, generateTaskDetailMarkdown as generateTaskDetailFromCore } from '@mapctx/core';
...
export class MarkdownKanbanParser {
  ...
  private static parseTaskProperty(line: string, task: KanbanTask): boolean {
    ...
    switch (propertyName) {
      case 'id':
        ...
        break;
      case 'type':
        if (['epic', 'feature', 'task', 'bug', 'chore'].includes(value)) {
          task.type = value as 'epic' | 'feature' | 'task' | 'bug' | 'chore';
        }
        break;
      case 'parent':
        if (value && value !== 'null') {
  ...
  private static generateTaskProperties(task: KanbanTask): string {
    ...
    if (task.parent) {
      properties += `  - parent: ${task.parent}\n`;
    }
    if (task.subIssueProgress) {
      properties += `  - subIssueProgress: ${task.subIssueProgress}\n`;
    }
  ...
  static parseTaskDetailMarkdown(content: string): Pick<KanbanTask, 'steps' | 'description'> {
    return parseTaskDetailFromCore(content);
  }

  static generateTaskDetailMarkdown(task: KanbanTask): string {
    return generateTaskDetailFromCore(task);
  }
