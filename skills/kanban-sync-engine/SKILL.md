...
description: Safely operate kanban-sync-engine for TASKS.md <-> GitHub synchronization; trigger when sessions start with kanban-sync-engine config, when users request pull/push/bootstrap/reconcile, or when divergence/conflicts must be diagnosed.
---
...
- Session starts and repository has `kanban-sync-engine.config.json`.
...
1. Run preflight for tool availability.
   - Check `command -v kanban-sync-engine`.
   - If unavailable, prefer non-invasive execution with `npx -y kanban-sync-engine@latest <command>`.
   - Suggest global install (`npm install -g kanban-sync-engine`) only with explicit user confirmation.

2. Check current sync state first.
   - Run `kanban-sync-engine status` (or npx fallback).

3. Refresh local base before any publish operation.
   - If remote differs or staleness is unknown, run `kanban-sync-engine pull` first.
...
5. Publish changes only after a successful pull in the active session.
   ...
   - Re-run `kanban-sync-engine status` to confirm convergence.
